...
```{block2, type = 'note'}
**TO DO**: $\hat{\beta}_0^{\tau}$ step function의 knots에 해당하는 $\tau$값들에 대해서는 예측변수에 해당하는 회귀계수가 예상과 다르게 추정되었다. $\tau$가 step function의 knots에 해당할 때의 quantile regression 추정에 대해서는 저자가 충분히 알지 못하여, 추후 공부한 내용을 보강하고자 한다.
```
