...
- Observable utility
\begin{eqnarray*}
v_{i1} &=& 0 - 0.001 * x_{i1} + 0.2 * z_{i}\\
v_{i2} &=& 4 - 0.001 * x_{i2} + 0.1 * z_{i}\\
v_{i3} &=& 2 - 0.001 * x_{i3} + 0.2 * z_{i}\\
...
- Availability
\begin{eqnarray*}
a_{i1} $\sim$ Bernoulli(0.95)\\
a_{i2} $\sim$ Bernoulli(0.95)\\
a_{i3} $\sim$ a_{i1} \times Bernoulli(0.9)\\
a_{i4} $\sim$ a_{i2} \times Bernoulli(0.9)
\end{eqnarray*}
...
generate_availability <- function(N) {
  tibble(
    ...
    avail1 = rbern(N, 0.95),
    avail2 = rbern(N, 0.95),
    avail3 = avail1 * rbern(N, 0.9),
    avail4 = avail2 * rbern(N, 0.9)
  ) %>%
    pivot_longer(
      cols = avail1:avail4, 
      names_to = "alternative", 
...
```{r}
choice_df %>% filter(choice == 1L) %>% pull(menu_item) %>% table()
```
