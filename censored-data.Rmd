...
bind_rows(
  tibble(
    demand = c(0L:20L),
    cdf = ppois(demand, lambda_true),
    type = "True"
  ),
  tibble(
    demand = 0L:20L,
    cdf = ppois(demand, lambda_estimate),
    cdf_min = ppois(demand, lambda_estimate - 1.96 * lambda_se),
    cdf_max = ppois(demand, lambda_estimate + 1.96 * lambda_se),
    type = "Estimated"
  )
) %>%
  ggplot(aes(x = demand, y = cdf)) +
  geom_linerange(aes(ymin = cdf_min, ymax = cdf_max, color = type),
  ...
  labs(
    ...
    y = NULL,
    color = NULL,
    title = "Estimated vs. true demand distribution (CDF)"
  ) +
  theme(
...
bind_rows(
  tibble(
    ...
    cdf = pexp(price, lambda_true),
    type = "True"
  ),
  tibble(
    price = seq(0.1, 20, by = 0.1),
    cdf = pexp(price, lambda_estimate),
    cdf_min = pexp(price, lambda_estimate + 1.96 * lambda_se),
    cdf_max = pexp(price, lambda_estimate - 1.96 * lambda_se),
    type = "Estimated"
  )
) %>%
  ggplot(aes(x = price, y = cdf)) +
  geom_line(aes(color = type)) +
  geom_ribbon(aes(ymin = cdf_min, ymax = cdf_max, fill = type),
              data = . %>% filter(type == "Estimated"),
